...
export const UNTAGGED_TAG: MemoTag = {
  id: "untagged",
  name: "タグなし",
  color: "#6b7280",
};
...
export const createMemoId = (date: string, title: string) =>
  `memo-${toIdDate(date)}-${toIdSegment(title)}`;

export const createTagId = (date: string, name: string) =>
  `tag-${toIdDate(date)}-${toIdSegment(name)}`;
...
export const createUniqueMemoId = (
  date: string,
  title: string,
  existingMemos: Pick<Memo, "id">[],
) =>
  createUniqueId(
    createMemoId(date, title),
    existingMemos.map((memo) => memo.id),
  );

export const createUniqueTagId = (
  date: string,
  name: string,
  existingTags: Pick<MemoTag, "id">[],
) =>
  createUniqueId(
    createTagId(date, name),
    existingTags.map((tag) => tag.id),
  );

const TAG_PRODUCT: MemoTag = {
  id: "tag-20260501-Product",
  name: "Product",
  color: "#2563eb",
};
const TAG_RESEARCH: MemoTag = {
  id: "tag-20260501-Research",
  name: "Research",
  color: "#059669",
};
const TAG_MEETING: MemoTag = {
  id: "tag-20260501-Meeting",
  name: "Meeting",
  color: "#dc2626",
};
const TAG_IDEA: MemoTag = {
  id: "tag-20260501-Idea",
  name: "Idea",
  color: "#7c3aed",
};
const TAG_PERSONAL: MemoTag = {
  id: "tag-20260501-Personal",
  name: "Personal",
  color: "#ea580c",
};
...
export const findMemoById = (id: string) =>
...
export const searchMemos = ({
  ...
  const queryTagIds = searchWords.flatMap((word) => knownTags.get(word) ?? []);
  const contentWords = searchWords.filter((word) => !knownTags.has(word));
  const requiredTagIds = [...new Set([...selectedTagIds, ...queryTagIds])];
  ...
  return targetMemos.filter((memo) => {
    ...
    const matchesWord =
      ...
      contentWords.every((word) => searchableText.includes(word));

    const matchesTags =
      requiredTagIds.length === 0 ||
      requiredTagIds.every((tagId) =>
        memo.tags.some((tag) => tag.id === tagId),
      );
...
export const groupMemoListByTag = (
  targetMemos: Memo[],
  targetTags: MemoTag[],
) => [
  ...targetTags.map((tag) => ({
    tag,
    memos: targetMemos.filter((memo) =>
      memo.tags.some((memoTag) => memoTag.id === tag.id),
    ),
  })),
