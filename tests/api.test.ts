...
declare global {
  ...
  var defineEventHandler: <TResponse>(
  ...
  // eslint-disable-next-line no-var
  var getHeader: (event: TestEvent, name: string) => string | undefined;
}
...
globalThis.getHeader = () => undefined;
...
class MemoryD1PreparedStatement {
  ...
  async first<T>() {
    return (
      (queryAll(this.db, this.sql, this.values)[0] as T | undefined) ?? null
    );
  }
...
const queryAll = (
  ...
  if (normalizedSql.includes("from memos m")) {
    ...
    const memoId = normalizedSql.includes("where m.id = ?")
      ? String(values[0])
      : undefined;
    const targetMemos = database.memos
      .filter(
        (memo) => memo.ownerUid === ownerUid && (!memoId || memo.id === memoId),
      )
      .sort(
        (a, b) =>
    ...
    return targetMemos.flatMap((memo) => {
      const memoTagRows = database.memoTags.filter(
        (memoTag) => memoTag.memoId === memo.id,
      );
      if (memoTagRows.length === 0) {
        return [memoRow(memo, null)];
      }

      return memoTagRows.map((memoTag) =>
        memoRow(
          memo,
          database.tags.find((tag) => tag.id === memoTag.tagId) ?? null,
        ),
      );
    });
  }
  ...
  if (
    normalizedSql.includes(
      "from tags where owner_uid = ? and lower(name) = lower(?)",
    )
  ) {
  ...
  if (normalizedSql.includes("from tags where id = ? and owner_uid = ?")) {
    ...
    return database.tags
      .filter((tag) => tag.id === id && tag.ownerUid === ownerUid)
      .map(tagRow);
  }
...
const runStatement = (
  ...
  if (normalizedSql.startsWith("delete from memo_tags")) {
    ...
    database.memoTags = database.memoTags.filter(
      (memoTag) => memoTag.memoId !== memoId,
    );
  ...
  if (normalizedSql.startsWith("insert or ignore into memo_tags")) {
    ...
    if (
      database.memoTags.some(
        (memoTag) => memoTag.memoId === memoId && memoTag.tagId === tagId,
      )
    ) {
      return 0;
    }
  ...
  if (normalizedSql.startsWith("update memos set")) {
    ...
    const memo = database.memos.find(
      (currentMemo) =>
        currentMemo.id === id && currentMemo.ownerUid === ownerUid,
    );
    if (!memo) {
      return 0;
    }
  ...
  if (normalizedSql.startsWith("delete from memos")) {
    ...
    database.memos = database.memos.filter(
      (memo) => memo.id !== id || memo.ownerUid !== ownerUid,
    );
    database.memoTags = database.memoTags.filter((memoTag) =>
      database.memos.some((memo) => memo.id === memoTag.memoId),
    );
  ...
  if (normalizedSql.startsWith("update tags set")) {
    ...
    const tag = database.tags.find(
      (currentTag) => currentTag.id === id && currentTag.ownerUid === ownerUid,
    );
    if (!tag) {
      return 0;
    }
  ...
  if (normalizedSql.startsWith("delete from tags")) {
    ...
    database.tags = database.tags.filter(
      (tag) => tag.id !== id || tag.ownerUid !== ownerUid,
    );
    database.memoTags = database.memoTags.filter(
      (memoTag) => memoTag.tagId !== id,
    );
...
const createTestDb = () => {
  ...
  database.memos = memos.map(({ tags: _tags, ...memo }) => ({
    ...memo,
    createdAt: memo.updatedAt,
  }));
  database.memoTags = memos.flatMap((memo) =>
    memo.tags.map((tag) => ({ memoId: memo.id, tagId: tag.id })),
  );
  return database;
};

const loadHandler = async <TResponse>(path: string) =>
  (await import(`${path}?test=${Date.now()}-${Math.random()}`))
    .default as ApiHandler<TResponse>;
...
const handlers: ApiHandlers = {
  ...
  getMemo: await loadHandler<MemoResponse>(
    "../server/api/memos/[id]/index.get.ts",
  ),
  updateMemo: await loadHandler<MemoResponse>(
    "../server/api/memos/[id]/index.patch.ts",
  ),
  deleteMemo: await loadHandler<DeleteResponse>(
    "../server/api/memos/[id]/index.delete.ts",
  ),
  ...
  getTag: await loadHandler<TagResponse>(
    "../server/api/tags/[id]/index.get.ts",
  ),
  updateTag: await loadHandler<TagResponse>(
    "../server/api/tags/[id]/index.patch.ts",
  ),
  deleteTag: await loadHandler<DeleteResponse>(
...
const event = (
  input: Omit<Partial<TestEvent>, "context"> & { authUid?: string | null } = {},
): TestEvent => ({
  body: input.body,
  params: input.params,
  query: input.query,
      ...
      : {
          firebaseUser: {
            uid:
              input.authUid ??
              String(
                (input.body as { ownerUid?: string } | undefined)?.ownerUid ??
                  input.query?.ownerUid ??
                  "demo-user-001",
              ),
          },
        }),
...
describe("memo API with D1", () => {
  test("GET /api/memos returns memos for owner", async () => {
    const response = await handlers.listMemos(
      event({ query: { ownerUid: "demo-user-001" } }),
    );

    assert.equal(response.memos.length, 6);
    assert.equal(response.memos[0].id, "memo-20260512-タグ検索-UI-の検討");
    assert.equal(
      response.memos.every((memo) => memo.ownerUid === "demo-user-001"),
      true,
    );
  });

  test("GET /api/memos requires authentication", async () => {
    await assert.rejects(
      handlers.listMemos(event({ authUid: null })),
      (error) => {
        const httpError = error as HttpError;
        return httpError.statusCode === 503 || httpError.statusCode === 401;
      },
    );
  });

  test("POST /api/memos ignores a forged ownerUid and uses authenticated uid", async () => {
    const response = await handlers.createMemo(
      event({
  ...
  test("GET /api/memos/{memoId} returns a memo and 404s for missing id", async () => {
    const response = await handlers.getMemo(
      event({
        params: { id: "memo-20260512-タグ検索-UI-の検討" },
        query: { ownerUid: "demo-user-001" },
      }),
    );
    ...
    await assert.rejects(
      handlers.getMemo(
        event({
          params: { id: "memo-20260516-missing" },
          query: { ownerUid: "demo-user-001" },
        }),
      ),
      (error) => {
        ...
        return (
          ...
          httpError.statusMessage === "Memo not found"
        );
      },
    );
  });
  ...
  test("DELETE /api/memos/{memoId} removes a memo", async () => {
    ...
    const storedMemo = await handlers
      .getMemo(
        event({
          params: { id: targetId },
          query: { ownerUid: "demo-user-001" },
...
describe("tag API with D1", () => {
  test("GET /api/tags returns owner tags", async () => {
    const response = await handlers.listTags(
      event({ query: { ownerUid: "demo-user-001" } }),
    );

    assert.equal(response.tags.length, 5);
    assert.equal(
      response.tags.some((tag) => tag.id === "tag-20260501-Product"),
      true,
    );
  });
  ...
  test("POST /api/tags creates a tag using tag-{date}-{name} id", async () => {
    const request = event({
      body: { ownerUid: "demo-user-001", name: "Design System" },
    });
  ...
  test("POST /api/tags returns existing tag for duplicate name", async () => {
    const response = await handlers.createTag(
      event({ body: { ownerUid: "demo-user-001", name: "Product" } }),
    );
  ...
  test("GET /api/tags/{tagId} returns a tag and 404s for missing id", async () => {
    const response = await handlers.getTag(
      event({
        params: { id: "tag-20260501-Product" },
        query: { ownerUid: "demo-user-001" },
      }),
    );
    assert.equal(response.tag.name, "Product");
    ...
    await assert.rejects(
      handlers.getTag(
        event({
          params: { id: "tag-20260516-missing" },
          query: { ownerUid: "demo-user-001" },
        }),
      ),
      (error) => {
        ...
        return (
          httpError.statusCode === 404 &&
          httpError.statusMessage === "Tag not found"
        );
      },
    );
  });
  ...
  test("DELETE /api/tags/{tagId} removes tag and detaches it from memos", async () => {
    ...
    assert.deepEqual(response, { deleted: true });
    assert.equal(
      memoResponse.memo.tags.some((tag) => tag.id === targetId),
      false,
    );
  });
});
